...
int insert_item(int item){
    nbItems++;
    if(nbItems  >=  1025 ){ //the producers have inserted all the items 
        ...
        if(poison_kills_reamining >= produce_not_finished){ //they are more poisonkills left to insert than working producer threads
            poisonkills++;
...
void* producer(void* arg){
    ...
    while(TRUE)
    {
        ...
        sem_wait(&empty); //waiting for a empty entry in the buffer
        pthread_mutex_lock(&mutex);
            // critical section (inserting)
            if(insert_item(item) == -1){ //producer thread doesn't have to insert anything more
                pthread_mutex_unlock(&mutex);
                break;
            }
        pthread_mutex_unlock(&mutex);
        sem_post(&full); //notifying that a new entry in the buffer has been filled
    }
}
...
void* consumer(void* arg){
    int item;
    while(TRUE){
        sem_wait(&full); //waiting for a filled entry in the buffer
        pthread_mutex_lock(&mutex);
        ...
        pthread_mutex_unlock(&mutex);
        sem_post(&empty); //notifying that a new empty entry is available in the buffer
...
int main(int argc, char const *argv[]){
    ...
    if(pthread_mutex_init(&mutex, NULL) !=0){error(-1, "init mutex error");}
    if(sem_init(&empty, 0, buffersize)!=0) {error(-1,"sem_init empty");}
    if(sem_init(&full, 0, 0)!=0) {error(-1,"sem_init full");}
    ...
    if(pthread_mutex_destroy(&mutex)!=0){error(-1,"mutex destroy");}
    if(sem_destroy(&empty)!=0) {error(-1,"sem_destroy empty");}
    if(sem_destroy(&full)!=0) {error(-1,"sem_destroy full");}
