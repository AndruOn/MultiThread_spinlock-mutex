...
int main(int argc, char const *argv[])
{
    ...
    baguette = malloc(sizeof(pthread_mutex_t) * numberOfBaguette);
    ...
    for (size_t i = 0; i < numberOfBaguette; i++){
        if(pthread_mutex_init(&(baguette[i]), NULL)!=0){error(err,"mutex_init");}
    }
    ...
    //destroy mute
    for (size_t i = 0; i < numberOfBaguette; i++){
        if(pthread_mutex_destroy(&(baguette[i]))!=0){error(err,"mutex destroy");}
    }

    // free pointeurs that were malloc
    free(baguette);
    free(index);
